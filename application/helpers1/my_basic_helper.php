...
if (!function_exists('my_check_subscription')) {
	function my_check_subscription($user_ids) {  //subscription's sn
		$CI = &get_instance();
		$query = $CI->db->where('user_ids', $user_ids)->where('status!=', 'expired')->where('end_time>=', my_server_time('UTC', 'Y-m-d'))->join('payment_item', 'payment_item.ids = payment_subscription.item_ids', 'LEFT')->get('payment_subscription', 1);
		if ($query->num_rows()) {
			return $query->row();
		}
		else {
