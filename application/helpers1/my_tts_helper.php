...
if (!function_exists('get_subscription_info')) {
	function get_subscription_info($user_ids = '') {
		...
		$query_subscription = $CI->db->where('user_ids', $user_ids)->where('status!=', 'expired')->where('end_time>=', my_server_time('UTC', 'Y-m-d'))->get('payment_subscription');
		if ($query_subscription->num_rows()) {
			$rs_subscription = $query_subscription->result()[0];
