...
class Query extends MY_Controller {
	...
	public function users() {
		...
		$this->m_datatables->set_response_fields_array(array('status', 'created_time', 'email_address', ['first_name', 'last_name'], 'ids'));
	...
	public function payment_subscription_list() {  //for admin use
		...
		if ($query->num_rows() && !empty($res_array['data'])) {
			...
			foreach ($res_array['data'] as $arr) {
				...
				$res_array['data'][$i][1] = $arr[1];  //replace the user_ids with user_email
				$res_array['data'][$i][2] = $subscription_item_array[$arr[2]];  //replace the item_ids with item_name
				$i++;
			}
		}
		echo json_encode($res_array);
	...
	public function tts_list() {  // for user and admin
		...
		$this->m_datatables->set_response_fields_array(array('created_time', 'language_name', 'voice_name', 'text', 'ids', 'tts_uri'));
		$res_array = $this->m_datatables->generate_data(my_get('draw'), my_get('start'), my_get('length'), $keyword);
		$i = 0;
		foreach ($res_array['data'] as $arr) {  //rebuild res_array
			(substr($arr[5], 0 ,3) != 'http') ?	$tts_uri = base_url($arr[5]) : $tts_uri = $arr[5];
			$res_array['data'][$i][4] = $arr[4] . ',' . $tts_uri;
