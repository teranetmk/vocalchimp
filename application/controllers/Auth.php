...
class Auth extends CI_Controller {
	...
	public function signup() {
		...
		if ($this->setting->signup_enabled) {  //if sign up from social media
			...
			if (get_cookie('vd_email')) {
				$data['vd_email'] = base64_decode(str_replace('FRZcehmrn', '', get_cookie('vd_email')));
			}
	...
	private function delete_user_from_mc($email = '')
	{
		$username = 'vocalchimp2'; // mailchimp
     	$password = 'c466a425c3597cd0b87901b2a65244b1-us6'; // mailchimp
	...
	public function signup_action() {
		...
		if ($this->setting->signup_enabled) {
			$this->form_validation->set_rules('first_name', my_caption('signup_first_name_label'), 'trim|required|max_length[100]');
			$this->form_validation->set_rules('last_name', my_caption('signup_last_name_label'), 'trim|required|max_length[100]');
			$this->form_validation->set_rules('email_address', my_caption('global_email_address'), 'trim|required|valid_email|max_length[100]|callback_email_duplicated_check');
			$user_row = $this->db->where('email_address', my_post('email_address'))->get('user', 1)->result();
			
			if (!empty($user_row)) {
				$this->session->set_flashdata('flash_danger', 'This email address is already registered. Login or <a href="/auth/forget">Reset your password</a>.');
			...
			//$result = $this->delete_user_from_mc($email);
			...
			if (!empty(my_post('password'))) {
				...
				$condition = 'trim|required|min_length[' . $min_length . ']|max_length[100]|callback_password_strength[' . $this->setting->psr . ']';
			}
			else {
				$condition = 'trim|required';
			}
			$this->form_validation->set_rules('password', my_caption('signup_password_label'), $condition);
			$this->form_validation->set_rules('confirm_password', my_caption('signup_confirm_label'), 'trim|required|min_length[6]|max_length[100]|matches[password]');
			$data = [];
			...
			if ( $this->form_validation->run() == FALSE) {
				...
				$this->session->set_flashdata('flash_danger', 'validation error. please try again!');
				redirect(base_url('/auth/signup'));

			} else {
				$username = my_post('email_address');
				$password = my_post('password');
				...
				if (!$check_invitation_pass) {
					...
					if ($res['result']) {  //sign up success
						...
						$this->user_model->signin_check($username, $password);
						
						return redirect(site_url() . $redirect);
					}
					else {
	...
	public function signin_action() {
		//echo my_hash_password('vocalchimp@123');die;
