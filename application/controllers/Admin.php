...
class Admin extends MY_Controller {
		...
		if ($query->num_rows()) {
			if ($query->row()->id == 1) {  //first super admin can not be removed
				...
				if ($query_subscription->num_rows()) {
					...
					my_payment_gateway_subscription_action('cancel_now', $query_subscription->row(), '/admin/CallBack');
	...
	public function edit_user() {
		...
		if ($query->num_rows()) {
			...
			$rs_payg = my_tts_generate_user_statitics(my_uri_segment(3));
	...
	public function payment_subscription_action() {
		(!my_check_permission('Payment Management')) ? die(my_caption('global_not_enough_permission')) : null; //check permission
		my_check_demo_mode('alert_json');  //check if it's in demo mode
		$pids = my_uri_segment(4);
		if(my_get('userid')) {
			$query = $this->db->where('user_ids', $pids)->where('status', 'active')->get('payment_subscription', 1);
		} else {
			$query = $this->db->where('ids', $pids)->get('payment_subscription', 1);
		}
	...
	public function payment_add_purchase() {
		...
		if ($query->num_rows()) {
			...
			$data['rs_purchase'] = $this->db->where('enabled', 1)->where('trash', 0)->where('type', 'purchase')->get('payment_item')->result();
	...
	public function payment_item_add_action() {
		...
		$this->form_validation->set_rules('characters_limit', my_caption('tts_payment_characters_limit_standard'), 'trim|integer|greater_than_equal_to[1]');
	...
	public function payment_item_remove_action() {
		...
		if ($query->num_rows()) {
			...
			if ($rs->type = 'subscription') {  //if the type of the item is subscription, need to check wheather have active subscription
				...
				$query_voice_resource = $this->db->like('accessibility_standard', $rs->ids)->or_like('accessibility_neural', $rs->ids)->get('tts_resource', 1);
				if ($query_subscription->num_rows()) {
					...
					if ($query_voice_resource->num_rows()) {
						$this->db->where('ids', my_uri_segment(3))->update('payment_item', array('trash'=>1));
					}
					else {
				...
				$query_purchased = $this->db->where('item_ids', $rs->ids)->get('payment_purchased', 1);
				if ($query_purchased->num_rows() || $query_voice_resource->num_rows()) {
					$this->db->where('ids', my_uri_segment(3))->update('payment_item', array('trash'=>1));
				}
				else {
					$this->db->where('ids', my_uri_segment(3))->delete('payment_item');
		...
		if ($query->num_rows()) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if (!$subscription_query->num_rows()) {
					$this->admin_model->payment_item_modify($rs->type);
	...
	public function print_my_path() {
		(!my_check_role('Super Admin')) ? die(my_caption('global_not_enough_permission')) : null; //check permission
		echo 'You script\'s full path is: <strong>'. FCPATH . '</strong>. Generally, you should store the configuration file outside the last directory and fill in the full path of the configuration file.';
	}
...
// $x = $stripe->promotionCodes->all(['limit' => 10]);
// prd($x); exit;
		$data['promocodes'] = $this->db->get('promocodes')->result();
		my_load_view($this->setting->theme, 'Admin/promocode', $data);
	}

	public function promocode_action() {
		$payment_setting_array = json_decode($this->setting->payment_setting, 1);
		$percent_off = my_post('percent_off');
		$promocode = my_post('promocode');
		$apiKey = $payment_setting_array['stripe_secret_key'];
		...
		$this->db->insert('promocodes', ['couponid' => $couponCode, 'code' => $promocode,
				'percent_off' => $percent_off]);
	...
	public function promocode_delete() {
		$payment_setting_array = json_decode($this->setting->payment_setting, 1);
		$apiKey = $payment_setting_array['stripe_secret_key'];
		$coupon_id = my_post('couponid');
		$stripe = new \Stripe\StripeClient($apiKey);

		try{
			$x = $stripe->coupons->delete($coupon_id, []);
		}catch(Exception $e){
			log_message('error',$e->getMessage());
		}
		
		$dp = $this->db->where('couponid', $coupon_id)->get('promocodes', 1)->row();
		$id = $dp->id;
		$this->db->delete('promocodes', ['id' => $id]);
		redirect('/admin/promocode');
	}
