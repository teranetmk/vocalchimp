...
class Home extends CI_Controller {
	...
	public function pricing() {
		...
		$data['payment_gateway_paypal_one_time'] = '';
		$data['payment_gateway_paypal_recurring'] = '';
		...
		if (!empty($_SESSION['access_code'])) {
			$query = $this->db->where('enabled', 1)->where('trash', 0)->where('access_code', $_SESSION['access_code'])->order_by('id', 'desc')->get('payment_item');
		}
		else {
			$query = $this->db->where('enabled', 1)->where('trash', 0)->where('access_code', '')->order_by('item_price', 'asc')->get('payment_item');
		}
		if ($query->num_rows()) {
