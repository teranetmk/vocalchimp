...
class Cronjob extends CI_Controller {
	...
	public function daily() {
		...
		//handle free subscription, subscribe automatically if it's necessary
		...
		//handle user's voice files
		$clean_up_setting = $this->db->get('tts_configuration', 1)->row()->clean_up;
		if ($clean_up_setting) {
			$current_time = now($this->config->item('time_reference'));
			$query = $this->db->where('created_time<', date('Y-m-d H:i:s', strtotime('-' . $clean_up_setting . ' days', $current_time)))->get('tts_log');
			if ($query->num_rows()) {
				$this->load->library('m_tts');
				$this->m_tts->deleteObject($query->result());  //bulk operation is handled at library
				$this->db->where('created_time<', date('Y-m-d H:i:s', strtotime('-' . $clean_up_setting . ' days', $current_time)))->delete('tts_log');
			}
		}
	...
	public function quarterly() {
		$query = $this->db->where('online', 1)->where('online_time<', date('Y-m-d H:i:s', strtotime('-1 minutes', now($this->config->item('time_reference')))))->get('user');
		if ($query->num_rows()) {
			$rs = $query->result();
			foreach ($rs as $row) {
		...
		// remove preview files evey quarter
		$query = $this->db->where('created_time<', date('Y-m-d H:i:s', strtotime('-30 minutes', now($this->config->item('time_reference')))))->get('tts_preview_log');
		if ($query->num_rows()) {
			$rs = $query->result();
			foreach ($rs as $row) {
				try {unlink(FCPATH . $row->tts_uri);} catch(\Exception $e) {} //remove voice file
				$this->db->where('id', $row->id)->delete('tts_preview_log');
			}
		}
		echo 'success';
	}
