...
<div class="container-fluid">
  <div class="row">
    ...
    <div class="col-lg-5 text-right">
	  <a href="https://support.cyberbukit.com/help-center/articles/23/specific-setting-for-tts-payment-package" target="_blank" class="btn btn-primary mr-2"><?=my_caption('global_help_documentation')?></a>
    </div>
  </div>
	    ...
	    <div class="col-lg-9">
	      ...
	      <div class="card shadow mb-4">
			...
			<div class="card-body">
			  ...
			  <div class="row form-group mb-4">
			    ...
			    <div class="col-lg-6">
				  ...
				  <?php
					...
					$options += my_get_all_payment_items(FALSE);
					$data = array(
					  'class' => 'form-control selectpicker'
					);
			  ...
			  <div class="row form-group mb-4" id="payment_item_row_characters_limit">
			    <div class="col-lg-6">
				  <label><?=my_caption('tts_payment_characters_limit_standard')?> <?=my_caption('tts_payment_notice_for_type')?></label>
				  <?php
				    if ($method == 'payment_item_add' || $method == 'payment_item_add_action') {
						$characters_limit = set_value('characters_limit');
					}
					else {
						$stuff_setting_array = json_decode($rs->stuff_setting, TRUE);
						(set_value('characters_limit') == '') ? $characters_limit = $stuff_setting_array['characters_limit'] : $characters_limit = set_value('characters_limit');
					}
					$data = array(
					  'type' => 'number',
					  'name' => 'characters_limit',
					  'id' => 'characters_limit',
					  'value' => $characters_limit,
					  'placeholder' => my_caption('tts_payment_notice_characters_unlimited'),
					  'class' => 'form-control'
					);
					echo form_input($data);
					echo form_error('characters_limit', '<small class="text-danger">', '</small>');
				  ?>
				</div>
			    <div class="col-lg-6">
				  <label><?=my_caption('tts_payment_characters_limit_neural')?> <?=my_caption('tts_payment_notice_for_type')?></label>
				  <br>
				  <div class="mt-1"><?=my_caption('tts_payment_notice_for_characters_limit_neural')?></div>
				</div>
			  </div>
			  <div class="row form-group mb-4" id="payment_item_row_actions">
			    <div class="col-lg-6">
				  <label><?=my_caption('global_bulk_actions')?> <?=my_caption('tts_bulk_action_notice')?></label>
				  <?php
					(set_value('bulk_actions') == '') ? $bulk_actions = '0' : $bulk_actions = set_value('bulk_actions');
					$options = array(
					  '0' => my_caption('global_none'),
					  '1' => my_caption('payment_bulk_actions_1'),
					  '2' => my_caption('payment_bulk_actions_2'),
					  '3' => my_caption('payment_bulk_actions_3'),
					  '4' => my_caption('payment_bulk_actions_4'),
					  '5' => my_caption('payment_bulk_actions_5'),
					  '6' => my_caption('payment_bulk_actions_6')
					);
					$data = array(
					  'id' => 'bulk_actions',
					  'class' => 'form-control'
					);
					echo form_dropdown('bulk_actions', $options, $bulk_actions, $data);
				  ?>
				</div>
			    <div class="col-lg-6">
