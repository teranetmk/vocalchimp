...
class User_model extends CI_Model {
	...
	public function get_user($user_ids) {
		return $this->db->where('ids', $user_ids)->get('user', 1)->row(1);
	}
	...
	public function signin_check($username='', $password='') {
		if (!empty($_SESSION['two_factor_authentication'])) {
			...
			if ($username != '') {
				$rs = $this->signin_query_user($username, $password);
			} else {
		...
		if ($query) {
			if ($query->num_rows()) {  //update the exist one
				...
				$insert_data = array(
				  ...
				  'coupon' => $coupon_code,
				  'coupon_discount' => $rs_item->item_price - $amount,
				  'tax' => $tax
				);
