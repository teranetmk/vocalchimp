...
if (!function_exists('my_check_signin')) {
	function my_check_signin() {
		if (!isset($_SESSION['user_ids'])) {
			...
			if (strpos($currenturl, 'user/pay') !== false) {
...
if (!function_exists('my_card')) {
	function my_card($type, $info, $color=null) {
		$data = '<div class="card mb-4 py-3 border-left-'. $type . '">';
		  $data .= '<div class="card-body text-center">';
		    $data .= ($color) ? "<span style='color:red'>$info</span>" : $info;
		  $data .= '</div>';
		$data .= '</div>';
		return $data;
	}
...
if (!function_exists('my_remove_user')) {
	function my_remove_user($user_ids) {
		...
		if ($rs->id != 1) {
			...
			$user = $CI->db->where('ids', $user_ids)->get('user')->row(); //user table
			$CI->db->where('ids', $user_ids)->delete('user'); //user table

			if(isset($user->stripecxid)){
				...
				$stripe->customers->delete($user->stripecxid,[]);
			}
		}
		return TRUE;
	}
						  ...
						  if (!empty($menu['child_menu'])) {
							  ...
							  foreach ($menu['child_menu'] as $child_menu) {
								  if ($child_menu['display']) {
									  ...
									  if($child_menu['name'] == "Cancel Subscription") {
										
										$active_subscription_id = $CI->db->where('status', 'active')->where('user_ids', $_SESSION['user_ids'])->get('payment_subscription')->row()->ids;
										if($active_subscription_id)
											$menu_output .= '<a class="collapse-item' . $child_active . '" href="#" onclick="cancel_user_subscription(\''.$active_subscription_id.'\')">' . $child_menu['name'] . '</a>';
									  }else {
										$menu_output .= '<a class="collapse-item' . $child_active . '" href="' . base_url($child_menu['link']) . '">' . $child_menu['name'] . '</a>';
									  }
							  ...
							  $menu_output .= '<li class="nav-item' . $active . '"><a class="nav-link" href="'. base_url($menu['link']) . '"><i class="' . $menu['icon'] . '"></i> <span>' . $menu['name'] . '</span></a></li>';
						  }
					  }
				  }
			  }
...
if (!function_exists('my_payment_gateway_subscription_action')) {
	function my_payment_gateway_subscription_action($action, $rs,$redirect_uri = null) {
		$CI = &get_instance();
		$result = FALSE;
		$payment_setting_array = json_decode($CI->setting->payment_setting, 1);
...
if (!function_exists('my_check_subscription')) {
	function my_check_subscription($user_ids) {  //subscription's sn
		$CI = &get_instance();
		$query = $CI->db->where('payment_subscription.user_ids', $user_ids)->where('payment_subscription.status!=', 'expired')->where('payment_subscription.status!=', 'incomplete')->where('end_time>=', my_server_time('UTC', 'Y-m-d'))->join('payment_item', 'payment_item.ids = payment_subscription.item_ids', 'LEFT')->get('payment_subscription', 1);
		if ($query->num_rows()) {
			return $query->row();
		}
		else {
...
if (!function_exists('my_get_catalog')) {
	function my_get_catalog($type, $user_ids='', $return_ids = FALSE) {
		$CI = &get_instance();
		if ($user_ids != '') {
			$query = $CI->db->where('user_ids', $user_ids);
		}
...
if (!function_exists('my_get_words')) {
	function my_get_words($sentence, $count = 10) {
		if (mb_detect_encoding($sentence, 'ASCII', true)) {
			...
			$result = mb_substr($sentence, 0, $count, 'utf-8');
		}
		return $result;
	}
}
...
if (!function_exists('my_kyc_check')) {
	function my_kyc_check($user_ids) {
		$CI = &get_instance();
		// $query = $CI->db->where('user_ids', $user_ids)->where('callback_status', 'success')->get('payment_log', 1);
		$query = $CI->db->where('user_ids', $user_ids)->where('redirect_status', 'success')->get('payment_log', 1);
		if ($query) {
	...
	function my_enhanced_safety() {
		...
		if (file_exists(FCPATH . 'vendor/phpunit')) {
			if (is_writable(FCPATH . 'vendor/phpunit')) {
				...
				$result = 'For safety reason, please remove the directory: "vendor/phpunit"';
			}
		}
		return $result;
	}
